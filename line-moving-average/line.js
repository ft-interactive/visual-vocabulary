...
function lineChart(data, stylename, media, yMin, yMax, yAxisHighlight, plotpadding,legAlign, logScale, logScaleStart, markers, numTicksy, yAlign, ticks,minAxis,threshold,precision){
    ...
    //use these to store input and output values
    var values = [];
    var output = {"values":[]};
    ...
    data.forEach(function(d,i){
        ...
        //parse data and calculate moving average
        values.push(+d.value);
        if (i>(threshold-2)){
        var avArray = [];
        for (var k=i;k>(i-threshold);k--){
          avArray.push(values[k])
        }
        var movAv = d3.mean(avArray)
        var result = {}
        result.date=d.date;
        result.value=parseFloat(movAv.toFixed(precision));
        output.values.push(result);
      }
    ...
    plot.append("g").attr("id","moving_average_line")
        .append("path")
        .datum(output)
        .attr("class",media+"lines")
        .attr("stroke","pink")
        .attr('d', function(d){
            return MAline(d.values); })
        .attr("transform",function(){
            if(yAlign=="right") {
                return "translate("+(margin.left)+","+(margin.top)+")"
            }
